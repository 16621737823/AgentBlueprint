    ...
    for (_, val_dict) in data_list.items():
        for (key, value) in val_dict.items():
            ...
            with open(conf.gen_path + f"desc_gen_{snake_key}.py", "w+") as file:
                ...
                file.write("            return \"\"\n")
                file.write("\n")
                file.write("    @model_validator(mode='after')\n")
    ...
    for (type, val_dict) in data_list.items():
        ...
        for (key, value) in val_dict.items():
            ...
            with open(conf.gen_path + f"mgr_gen_{snake_key}.py", "w+") as file:
                file.write("from data_module import QueryContext, DataInterface, DataListInterface, DataManagerInterface, DataNodeContext, FunctionNodeContext\n")
                ...
                if type == "ConnectorData":
                    ...
                    for (param_key, param) in value["parameters"].items():
                        ...
                        file.write(f"        if {snake_param_key}[1] is None or {snake_param_key}[2] is None:\n")
                    ...
                    file.write("\n")
                    file.write("    @staticmethod\n")
                    file.write("    def get_descriptor(desc_index:int, ctx:DataNodeContext) -> DataInterface or DataListInterface:\n")
                    ...
                    file.write(f"            return {key}Manager._previous(ctx, connected_params)\n")
                    ...
                    file.write("\n")
                    file.write("    @staticmethod\n")
                    file.write("    def _single(ctx: DataNodeContext,connected_params:dict = None) -> DataInterface or DataListInterface:\n")
                    ...
                    file.write("\n")
                    file.write("    @staticmethod\n")
                    file.write("    def _list(ctx: DataNodeContext,connected_params:dict= None) -> DataInterface or DataListInterface:\n")
                    file.write("        #TODO: implement me, this is where connects to a datasource, could be a database or a service\n")
                    file.write("        raise NotImplementedError\n")
                    file.write("\n")
                    file.write("    @staticmethod\n")
                    file.write("    def _previous(ctx: DataNodeContext,connected_params:dict= None) -> DataInterface or DataListInterface:\n")
                    ...
                    for desc_type in value["descriptor"] if value["descriptor"] is not None else []:
                        for (desc_key, desc) in desc_list[desc_type].items():
                            snake_desc_key = re.sub('([a-z0-9])([A-Z])', r'\1_\2', desc_key).lower()
                            file.write("    @staticmethod\n")
                            file.write(f"    def _{snake_desc_key}(ctx: DataNodeContext,connected_params:dict= None) -> DataInterface or DataListInterface:\n")
                            file.write("        #TODO: implement me, this is where connects to a datasource, could be a database or a service\n")
                    ...
                    file.write("\n")
                    file.write("    @staticmethod\n")
                    file.write("    def _single( ctx: DataNodeContext) -> DataInterface or DataListInterface:\n")
                    file.write("        #TODO: implement me, this is where connects to a datasource, could be a database or a service\n")
                    file.write("        raise NotImplementedError\n")
                    file.write("\n")
                    file.write("    @staticmethod\n")
                    file.write("    def _list(ctx: DataNodeContext) -> DataInterface or DataListInterface:\n")
                    file.write("        #TODO: implement me, this is where connects to a datasource, could be a database or a service\n")
                    file.write("        raise NotImplementedError\n")
                    file.write("\n")
                    file.write("    @staticmethod\n")
                    file.write("    def _previous(ctx: DataNodeContext) -> DataInterface or DataListInterface:\n")
                    file.write("        return ctx.get_reference_context(ctx.source_index)\n")
                    file.write("\n")
                    for desc_type in value["descriptor"] if value["descriptor"] is not None else []:
                        for (desc_key, desc) in desc_list[desc_type].items():
                            snake_desc_key = re.sub('([a-z0-9])([A-Z])', r'\1_\2', desc_key).lower()
                            file.write("    @staticmethod\n")
                            file.write(f"    def _{snake_desc_key}(ctx: DataNodeContext) -> DataInterface or DataListInterface:\n")
                            file.write("        #TODO: implement me, this is where connects to a datasource, could be a database or a service\n")
                            file.write(f"        raise NotImplementedError\n")
                            file.write("\n")
