        ...
        for (key, value) in val_dict.items():
            ...
            with open(conf.gen_path + f"mgr_gen_{snake_key}.py", "w+") as file:
                ...
                if type == "ConnectorData":
                    file.write("    def fetch_connected_data(self, ctx: QueryContext):\n")
                    ...
                    file.write("            return self._list(ctx, connected_params)\n")
                    for desc_type in value["descriptor"] if value["descriptor"] is not None else []:
                        for (desc_key, desc) in desc_list[desc_type].items():
                            ...
                            file.write(f"            return self._{snake_desc_key}(ctx, connected_params)\n")
                    file.write("        else:\n")
                    file.write("            raise ValueError(\"Invalid Descriptor Index\")\n")
                    file.write("\n")
                    file.write("    def _single(self, ctx: QueryContext,connected_params:dict = None) -> DataInterface or DataListInterface:\n")
                    ...
                    file.write("\n")
                    file.write("    def _list(self, ctx: QueryContext,connected_params:dict= None) -> DataInterface or DataListInterface:\n")
                    file.write("        #TODO: implement me, this is where connects to a datasource, could be a database or a service\n")
                    file.write("        raise NotImplementedError\n")
                    file.write("\n")
                    for desc_type in value["descriptor"] if value["descriptor"] is not None else []:
                        for (desc_key, desc) in desc_list[desc_type].items():
                            snake_desc_key = re.sub('([a-z0-9])([A-Z])', r'\1_\2', desc_key).lower()
                            file.write(f"    def _{snake_desc_key}(self, ctx: QueryContext,connected_params:dict= None) -> DataInterface or DataListInterface:\n")
                            file.write("        #TODO: implement me, this is where connects to a datasource, could be a database or a service\n")
